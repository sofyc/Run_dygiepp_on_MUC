...
types = {'kidnapping': 0, 'attack':1, 'bombing':2, 'robbery':3, 'arson':4, 'bombing / attack':5, 'attack / bombing':6, 'forced work stoppage':7}
    ...
    for file in files:
        ...
        with open(file) as f:
            ...
            for inst in instances:
                ...
                tokens = [['kidnapping', 'attack', 'bombing', 'robbery', 'arson', 'bombing / attack', 'attack / bombing', "forced work stoppage"] + tokens[0]]
                doc["sentences"] = tokens
                templates = inst["templates"]

                for template in templates:
                    roles = ["incident_type", "PerpInd", "PerpOrg", "Target", "Weapon", "Victim", "incident_date", "incident_location"]

                    for role in roles:
                        if role == "incident_type":
                            triple = [types[template[role]], types[template[role]], role]
                            entityWithRole[0].append(triple)
                        elif len(template[role]) != 0:
