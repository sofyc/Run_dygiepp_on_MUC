...
class NERTagger(Model):
    ...
    def predict(self, ner_scores, spans, span_mask, metadata):
        ...
        for ner_scores_sent, spans_sent, span_mask_sent, sentence in zipped:
            ner_max_score = 0
            for i in range(ner_scores_sent.shape[0]):
                if spans_sent[i, 0] <= 7 and spans_sent[i, 0] == spans_sent[i, 1]:
                    ner_scores_sent[i, 2:] = -1e20
                    if ner_scores_sent[i, 1] > ner_max_score:
                        ner_max_score = ner_scores_sent[i, 1]
                elif spans_sent[i, 0] >= 8:
                    break
            
            for i in range(ner_scores_sent.shape[0]):
                if spans_sent[i, 0] <= 7 and spans_sent[i, 0] == spans_sent[i, 1]:
                    if ner_scores_sent[i, 1] < ner_max_score:
                        ner_scores_sent[i, 1] = -1e20
                elif spans_sent[i, 0] >= 8:
                    break
